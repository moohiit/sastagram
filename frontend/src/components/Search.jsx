...
const GridSkeleton = () => (
  <div className='grid grid-cols-3 gap-1'>
    {Array.from({ length: 6 }).map((_, i) => (
      <div key={i} className='aspect-square animate-pulse rounded bg-zinc-200 dark:bg-zinc-800' />
    ))}
  </div>
);
...
const Search = () => {
  ...
  return (
    <div className='mx-auto w-full max-w-[470px] px-4 py-6'>
      <h1 className='mb-4 text-xl font-bold text-zinc-900 dark:text-gray-100'>Search</h1>
      ...
      <div className='relative mb-4'>
        ...
        <Input
          ...
          placeholder='Search'
          autoFocus
          className='rounded-lg border-zinc-200 dark:border-zinc-800 pl-10 pr-10 focus-visible:ring-transparent'
        />
        {query ? (
          <button
            onClick={() => setQuery('')}
            aria-label='Clear search'
            className='absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-400'
          >
            <X size={18} />
          </button>
        ) : null}
        ...
        {TABS.map(t => (
          <button
            ...
            className={`flex-1 cursor-pointer border-b-2 pb-2 text-sm font-semibold transition-colors ${
              tab === t.id
                ? 'border-zinc-900 dark:border-gray-100 text-zinc-900 dark:text-gray-100'
                : 'border-transparent text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-400'
            }`}
          >
            {t.label}
          </button>
      ...
      {tab === 'accounts' ? (
        showResults ? (
          <div className='rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black p-2'>
          ...
          <>
            ...
            <div className='mb-6'>
              <div className='mb-1 flex items-center justify-between px-1'>
                <h2 className='text-base font-semibold text-zinc-900 dark:text-gray-100'>Recent</h2>
                {recent.length > 0 ? (
                  <button
                    onClick={clearRecent}
              ...
              {recent.length === 0 ? (
                <p className='px-1 py-6 text-center text-sm text-zinc-500 dark:text-zinc-400'>No recent searches.</p>
              ) : (
                recent.map(u => (
                  <UserRow
                    key={u._id}
                    ...
                    action={
                      <button
                        ...
                        className='cursor-pointer p-1 text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-400'
                      >
                        <X size={18} />
                      </button>
                    }
            ...
            <>
              <div className='mb-2 flex items-center justify-between px-1'>
                {postMode === 'semantic' ? (
                  ...
                  <span className='rounded-full bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 text-xs text-zinc-500'>
                    basic search
                  </span>
                )}
                {postSearching ? (
              ...
              <div className='grid grid-cols-3 gap-1'>
                {postResults.map(post => (
                  <button
                    ...
                    className='group relative aspect-square cursor-pointer overflow-hidden bg-zinc-100 dark:bg-zinc-900 focus:outline-none'
                  >
                    <img
