...
function Chat() {
  ...
  return (
    <div className='flex h-[calc(100dvh-7.5rem)] md:h-[100dvh]'>
      ...
      <aside
        className={`${threadOpen ? 'hidden md:flex' : 'flex'} w-full md:w-[340px] lg:w-[380px] shrink-0 flex-col border-r border-zinc-200 dark:border-zinc-800`}
      >
        <ConversationList
      ...
      <section className={`${threadOpen ? 'flex' : 'hidden md:flex'} flex-1 min-w-0 flex-col`}>
        {groupId ? (
          ...
          <div className='flex flex-1 flex-col items-center justify-center px-4'>
            <div className='flex items-center justify-center h-24 w-24 rounded-full border-2 border-zinc-900 dark:border-gray-100'>
              <MessageCircle size={44} strokeWidth={1.5} />
            </div>
            <h2 className='text-xl font-bold text-zinc-900 dark:text-gray-100 mt-4'>Your messages</h2>
            <p className='text-sm text-zinc-500 dark:text-zinc-400 mt-1'>Send a message to start a chat.</p>
            <Button
