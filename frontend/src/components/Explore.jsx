...
function Explore() {
  ...
  return (
    <div className='max-w-4xl mx-auto p-4'>
      <h1 className='text-xl font-bold text-zinc-900 dark:text-gray-100 mb-4'>Explore</h1>

      {loading && (
        <div className='grid grid-cols-3 gap-1'>
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className='aspect-square animate-pulse bg-zinc-200 dark:bg-zinc-800 rounded' />
          ))}
        </div>
      )}
      ...
      {!loading && error && (
        <div className='flex flex-col items-center gap-2 py-16'>
          <p className='text-sm text-zinc-500 dark:text-zinc-400'>Couldn&apos;t load posts.</p>
          <button
            onClick={fetchPosts}
        ...
        <div className='grid grid-cols-3 gap-1'>
          {posts.map((post) => (
            <button
              ...
              className='group relative aspect-square overflow-hidden bg-zinc-100 dark:bg-zinc-900 focus:outline-none cursor-pointer'
            >
              <img
