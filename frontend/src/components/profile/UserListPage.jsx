...
const UserListPage = ({ type, title }) => {
  ...
  return (
    <div className='mx-auto w-full max-w-[470px] px-4 py-6'>
      <div className='rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black'>
        <div className='flex items-center gap-3 border-b border-zinc-200 dark:border-zinc-800 p-3'>
          <button
            onClick={() => navigate(-1)}
            aria-label='Go back'
            className='cursor-pointer rounded-lg p-1 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-900'
          >
            <ArrowLeft size={20} className='text-zinc-900 dark:text-gray-100' />
          </button>
          <h1 className='text-base font-semibold text-zinc-900 dark:text-gray-100'>{title}</h1>
        </div>
        <div className='p-2'>
          {users === null && !error ? (
            <>
